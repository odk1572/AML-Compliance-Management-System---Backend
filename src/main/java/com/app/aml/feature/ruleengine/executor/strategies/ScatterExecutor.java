...
public class ScatterExecutor implements RuleExecutorStrategy {
    ...
    public Set<UUID> executeRule(RuleExecutionContextDto rule) {
        ...
        // Ensure Scenario Context (A) encompasses Rule Window (B)
        ...
        // 4. The 2-Step SQL Logic (CTE for Context + Distinct Beneficiary Count)
        String sql = String.format("""
            -- STEP 1: Bound initial transactions by Scenario Lookback
            WITH scenario_context AS (
                SELECT originator_account_no, beneficiary_account_no, transaction_timestamp 
                FROM %s.transactions
                WHERE transaction_timestamp >= CURRENT_TIMESTAMP - CAST(? AS INTERVAL)
            )
            -- STEP 2: Apply Scatter Logic within the Rule Time Window
            ...
            GROUP BY cp.id 
            HAVING COUNT(DISTINCT t.beneficiary_account_no) >= ?
        """, schema, schema);

        log.debug("Executing Scatter Rule for tenant: {} (Lookback: {}, Window: {})",
        ...
        List<UUID> results = jdbcTemplate.query(sql,
                ...
                ruleTimeWindow,   // 2nd ?: Rule Filter
                targetCount       // 3rd ?: Distinct beneficiary threshold
        );
