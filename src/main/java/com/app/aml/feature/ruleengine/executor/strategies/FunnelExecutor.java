...
public class FunnelExecutor implements RuleExecutorStrategy {
    ...
    @Autowired
    public FunnelExecutor(JdbcTemplate jdbcTemplate) {
    ...
    public List<RuleBreachResult> executeRule(RuleExecutionContextDto rule) {
        ...
        if (targetCount <= 0 || chunkSize == null) {
            throw new IllegalStateException("Required parameters missing for Funnel Rule: " + rule.getTypologyLabel());
        }
        ...
        String sql = String.format("""
            ...
            breaching_customers AS (
                ...
                AND (
                    SELECT COUNT(DISTINCT t2.originator_account_no)
                    FROM data_universe t2
                    WHERE t2.beneficiary_account_no = t1.account_number
