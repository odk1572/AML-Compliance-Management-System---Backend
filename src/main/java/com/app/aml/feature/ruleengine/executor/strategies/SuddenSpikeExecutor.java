...
public class SuddenSpikeExecutor implements RuleExecutorStrategy {
    ...
    public List<RuleBreachResult> executeRule(RuleExecutionContextDto rule) {
        String shortInterval = null;
        String longInterval = null;
        ...
        for (ConditionExecutionContextDto cond : rule.getConditions()) {
            ...
            switch (aggFn) {
                case "SUM" -> shortInterval = cond.getLookbackPeriod();
                case "AVG" -> longInterval = cond.getLookbackPeriod();
                case "NONE" -> multiplier = new BigDecimal(cond.getThresholdValue());
            }
        }

        if (shortInterval == null || longInterval == null || multiplier == null) {
            ...
            breaching_customers AS (
                ...
                AND (
                    ...
                    (
                        ...
                        FROM data_universe t3
                        WHERE t3.originator_account_no = t1.account_number
                          AND t3.transaction_timestamp BETWEEN t1.transaction_timestamp - CAST(? AS INTERVAL) AND t1.transaction_timestamp - CAST(? AS INTERVAL)
                    ) / NULLIF(EXTRACT(EPOCH FROM (CAST(? AS INTERVAL) - CAST(? AS INTERVAL))) / 86400.0, 0)
                ) * ?
            )
            SELECT 
                ? as rule_type,
